...
// 
// level.additionalprimaryweapon_limit = int // changed amount of guns you can hold with mule kick
...
{
	self additional_primary_enabled(0);
	// self clientfield::set_player_uimodel( ADDITIONAL_PRIMARY_WEAPON_UI_GLOW_CLIENTFIELD, 0 );
}
...
function take_additionalprimaryweapon()
{
	...
	if( IS_TRUE( self._retain_perks ) || ( isdefined( self._retain_perks_array ) && IS_TRUE( self._retain_perks_array[ ADDITIONAL_PRIMARY_WEAPON_PERK ] ) ) ) {
	...
	while( pwtcbt >= 3 ) {
		...
		self.weapon_taken_by_losing_specialty_additionalprimaryweapon[weapon_to_take] = zm_weapons::get_player_weapondata( self, weapon_to_take );
		pwtcbt--;
