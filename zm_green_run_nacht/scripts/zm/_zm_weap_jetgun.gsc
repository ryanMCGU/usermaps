...
function jetgun_crafting_pick_up(parent_trig, weapon_model) {
	...
	for (;;) {
		...
		if (player zm_magicbox::can_buy_weapon() && (player GetCurrentWeapon() != level.weaponZMjetgun)) {
            ...
            player GiveWeapon(level.weaponZMjetgun);
            player SwitchToWeaponImmediate(level.weaponZMjetgun);
