...
function autoexec main() {
    thread handlePerk("jugg1", "specialty_staminup");
	thread handlePerk("jugg2", "specialty_phdflopper");
	thread handlePerk("jugg3", "specialty_deadshot");
	thread handlePerk("jugg4", "specialty_additionalprimaryweapon");
}

function handlePerk(perk, perkGive) {
	trigs = GetEntArray(perk + "_perk_trigger", "targetname");

    foreach(trig in trigs) {
        trig thread handleTrig(perk, perkGive);
    }
}

function handleTrig(perk, perkGive) {
    endCon = false;
