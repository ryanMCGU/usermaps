...
{	
	zm_perks::register_perk_basic_info( PERK_PHDFLOPPER, "phd", PHDFLOPPER_PERK_COST, "Hold ^3[{+activate}]^7 for PHD Flopper [Cost: &&1] \n ^8Immune to explosives, increases explosive damage. Dive to Prone from a height to cause an explosion.", GetWeapon( PHDFLOPPER_PERK_BOTTLE_WEAPON ) );
...
function phdflopper_activate() 
{
    ...
    if(IS_TRUE(PHDFLOPPER_EXPLOSIVE_DIVE))
    	self thread phd_flopper_explosive_dive();
...
function phd_flopper_explosive_dive()
{
    self endon("death");
    self endon("disconnect");
    self endon(PERK_PHDFLOPPER + "_stop");
...
function phd_flopper_explode() 
{
	...
	// Spawn FX Model
	origin = self GetEye();
	fxModel = util::spawn_model("tag_origin", origin );
