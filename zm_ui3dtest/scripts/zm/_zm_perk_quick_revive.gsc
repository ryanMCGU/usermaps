...
#precache( "string", QUICK_REVIVE_INFO_STRING );
...
function set_up_callbacks() {
	zm_spawner::register_zombie_death_event_callback( &extend_bleedout_each_kill );
}

function enable_quick_revive_perk_for_level()
{	
	// register quick revive perk for level
	zm_perks::register_perk_basic_info( PERK_QUICK_REVIVE, "revive", &revive_cost_override, QUICK_REVIVE_INFO_STRING, GetWeapon( QUICK_REVIVE_PERK_BOTTLE_WEAPON ) );
...
{
	// Faster Health regen
	self thread increase_health_regen();

	self thread give_grenades_in_laststand();
...
function increase_health_regen()
{
	// SELF == PLAYER
	
	self endon (PERK_QUICK_REVIVE + "_stop");
	...
	{	
		...
		{
			if ((self.maxhealth - self.health) > PLAYER_HEALTH_REGEN)
				self.health = self.health + PLAYER_HEALTH_REGEN;
	
			else
				self.health = self.maxhealth;
				
			////IPrintLnBold("HP = ", self.maxHealth, " Current HP = ", self.health);
			wait (PLAYER_REGEN_CYCLE_TIME);
		}
...
function give_grenades_in_laststand() {
	// SELF == PLAYER
	...
	//IPrintLnBold("Giving Lethals");
	w_tactical = self zm_utility::get_player_lethal_grenade();
	self GiveWeapon( w_tactical );
	self EnableOffhandWeapons();
	self zm_utility::set_player_lethal_grenade( w_tactical );
}

function extend_bleedout_each_kill( e_attacker ) {
	if ( zm_perks::use_solo_revive() ) {
		return;
	}
	...
	if (IS_TRUE(e_attacker.laststand) && isdefined(e_attacker.bleedout_time)) {
		e_attacker.bleedout_time += BLEED_OUT_KILL_BONUS;
		//IPrintLnBold("Extending Bleedout");
	}
	return;
}