...
#define ELECTRIC_CHERRY_INFO_STRING					"Hold ^3[{+activate}]^7 for Electric Cherry 2.0 [Cost: &&1] \n ^8 Electric damage now chains like deadwire (30 second cooldown)."
//                                                   
...
#define TESLA_LINK_COOLDOWN 30 // Cooldown for the insta kill lightning chain in seconds

#precache( "fx", "_t6/misc/fx_zombie_cola_revive_on" );
#precache( "fx", "dlc1/castle/fx_castle_electric_cherry_down" );
#precache( "string", ELECTRIC_CHERRY_INFO_STRING );
...
function __main__() {
	zm::register_actor_damage_callback(&tesla_damage_modifier);
}

function tesla_damage_modifier (e_inflictor, e_attacker, n_damage, b_flags, str_means_of_death, w_weapon, v_point, v_dir, str_hit_loc, n_ps_offset_time, n_bone_index, str_surface_type) {
	...
	//IPrintLnBold(w_weapon);
    if (w_weapon == GetWeapon( ELECTRIC_CHERRY_PERK_BOTTLE_WEAPON )) {
        self do_tesla_damage(b_death, e_attacker, str_means_of_death, w_weapon);
    }
...
{	
	...
	zm_perks::register_perk_basic_info( PERK_ELECTRIC_CHERRY, "electric_cherry", ELECTRIC_CHERRY_PERK_COST, ELECTRIC_CHERRY_INFO_STRING, GetWeapon( ELECTRIC_CHERRY_PERK_BOTTLE_WEAPON ) );
	...
	if ( IsDefined( self ) )
	{
		...
		for ( i = 0; i < a_zombies.size; i++ )
		{
			if ( IsAlive( self ) && IsAlive( a_zombies[ i ] ) )
			{
				...
				self.canTesla = true;
				a_zombies[ i ] DoDamage( ELECTRIC_CHERRY_DOWNED_ATTACK_DAMAGE, self.origin, self, self, "none", "MOD_RIFLE_BULLET", 0, GetWeapon( ELECTRIC_CHERRY_PERK_BOTTLE_WEAPON ));
			}
		}
		...
		{			
			...
			for ( i = 0; i < a_zombies.size; i++ )
			{
				if ( IsAlive( self ) && IsAlive( a_zombies[ i ] ) )
				{
					...
					{
						a_zombies[ i ] DoDamage( perk_dmg, self.origin, self, self, "none", "MOD_RIFLE_BULLET", 0, GetWeapon( ELECTRIC_CHERRY_PERK_BOTTLE_WEAPON ));
					}
				}
			}
...
//self == zombie
function do_tesla_damage(b_death, e_attacker, str_means_of_death, w_weapon) {
	if (IS_TRUE(e_attacker.canTesla)) {
		str_aat = "zm_aat_dead_wire";
    	self thread[[level.aat[str_aat].result_func]](b_death, e_attacker, str_means_of_death, w_weapon);
		e_attacker thread tesla_cooldown();
	}
}

function tesla_cooldown() {
	self endon( "death" );
	self endon( "disconnect" );
	self endon( PERK_ELECTRIC_CHERRY + "_stop" );

	//IPrintLnBold("Cooldown started");
	self.canTesla = false;
	wait TESLA_LINK_COOLDOWN;
	self.canTesla = true;
	//IPrintLnBold("Cooldown done");
}
