...
{
	if( isdefined( self.weapon_taken_by_losing_specialty_additionalprimaryweapon ) ) {
		self return_additionalprimaryweapon( self.weapon_taken_by_losing_specialty_additionalprimaryweapon );
		self ShouldDoInitialWeaponRaise( self.weapon_taken_by_losing_specialty_additionalprimaryweapon, 0 );
		self SwitchToWeaponImmediate( self.weapon_taken_by_losing_specialty_additionalprimaryweapon );
	}
}
...
function take_additionalprimaryweapon()
{
    if( IS_TRUE( self._retain_perks )
        || ( isdefined( self._retain_perks_array )
            && IS_TRUE( self._retain_perks_array[PERK_ADDITIONAL_PRIMARY_WEAPON] ) ) ) {
        return level.weaponNone;
    }
    valid_take_weapons = [];
    weapons_list = self GetWeaponsListPrimaries();
    for( i = 0; i < weapons_list.size; i++ ) {
        if( zm_weapons::is_weapon_included( weapons_list[i] )
            || zm_weapons::is_weapon_upgraded( weapons_list[i] ) ) {
            valid_take_weapons[valid_take_weapons.size] = weapons_list[i];
        }
    }
    if( valid_take_weapons.size < 3 ) {
        return level.weaponNone;
    }
    weapon = valid_take_weapons[valid_take_weapons.size - 1];
    if( !isdefined( weapon ) || weapon == level.weaponNone ) {
        return level.weaponNone;
    }
    if( !isdefined( self.weapons_taken_by_losing_specialty_additionalprimaryweapon ) ) {
        self.weapons_taken_by_losing_specialty_additionalprimaryweapon = [];
    }
    self.weapons_taken_by_losing_specialty_additionalprimaryweapon[weapon] = self zm_weapons::get_player_weapondata( self, weapon );
    if( self GetCurrentWeapon() == weapon && valid_take_weapons.size > 0 ) {
        self SwitchToWeapon( valid_take_weapons[0] );
    }
...
{
    if( !isdefined( w_returning ) || w_returning == level.weaponNone ) {
        return;
    }
    if( isdefined( self.weapons_taken_by_losing_specialty_additionalprimaryweapon[w_returning] ) ) {
        ...
        foreach( weapon in current_weapons ) {
            if( weapon == w_returning ) {
                ArrayRemoveValue( current_weapons, weapon, 0 );
            }
        }
        weapons_taken = [];
        for( i = 0; i < current_weapons.size; i++ ) {
            weapons_taken[i] = zm_weapons::get_player_weapondata( self, current_weapons[i] );
            self TakeWeapon( current_weapons[i] );
        }
        for( i = 0; i < weapons_taken.size; i++ ) {
            self zm_weapons::weapondata_give( weapons_taken[i] );
        }
...
{	
	...
	zm_perks::register_perk_basic_info( PERK_ADDITIONAL_PRIMARY_WEAPON, "additionalprimaryweapon", ADDITIONAL_PRIMARY_WEAPON_PERK_COST, "Hold ^3[{+activate}]^7 for Mule Kick 2.0 [Cost: &&1] \n ^8Gain a 3rd weapon slot, saves and returns a weapon on re-purchase", GetWeapon( ADDITIONAL_PRIMARY_WEAPON_PERK_BOTTLE_WEAPON ) );
	zm_perks::register_perk_precache_func( PERK_ADDITIONAL_PRIMARY_WEAPON, &additional_primary_weapon_precache );
	zm_perks::register_perk_clientfields( PERK_ADDITIONAL_PRIMARY_WEAPON, &additional_primary_weapon_register_clientfield, &additional_primary_weapon_set_clientfield );
	zm_perks::register_perk_machine( PERK_ADDITIONAL_PRIMARY_WEAPON, &additional_primary_weapon_perk_machine_setup );
	zm_perks::register_perk_threads( PERK_ADDITIONAL_PRIMARY_WEAPON, &give_additional_primary_weapon_perk, &take_additional_primary_weapon_perk );
...
function give_additional_primary_weapon_perk()
{
	//IPrintLnBold("Mule Given");
	if ( isdefined( self.weapons_taken_by_losing_specialty_additionalprimaryweapon ) && isdefined( self.weapons_taken_by_losing_specialty_additionalprimaryweapon[self.weapon_mule_taken] ) ) {

		self zm_weapons::give_build_kit_weapon( self.weapon_mule_taken );
		//IPrintLnBold("Weapon Given");
	}
}
...
function take_additionalprimaryweapon()
{
	...
	//IPrintLnBold("Slots = ", pwtcbt, "Slot to take = ", weapon_to_take.name);

	self.weapon_mule_taken = weapon_to_take;
