...
#precache( "fx", "fx\weapon\fx_betty_exp.efx" );
...
{	
    ...
    foreach (trigger in triggers) {
		...
		*/
	level._effect["betty_explode"] = "fx\weapon\fx_betty_exp.efx";
	level._effect["betty_trail"] = "";
...
function player_give_betty( str_weapon = "bo1_bouncingbetty" )
{
	...
	self zm_equipment::set_player_equipment( level.waw_betties );
...
function handleExplosion() {
	...
	//self thread handlePickupTrigger();

	wait(2);

	//self PlaySound("betty_activated"); //Need to add
	wait(.1);	
	...
	//PlayFXOnTag(level._effect["betty_trail"], tag_origin,"tag_origin"); //Need to add
