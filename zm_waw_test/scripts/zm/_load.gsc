...
function fps_command_response( command_args )
{
	oldvalue = undefined;
	for(;;) {
		dvar_value = GetDvarString( "com_maxfps", "" );
		if( isdefined( dvar_value ) && dvar_value != "" ) {
			if( oldvalue === dvar_value ) {
				wait .05;
				continue;
			}
			SetDvar( "com_maxfps", dvar_value );
		}
		oldvalue = dvar_value;
		wait .05;
	}
