...
function register()
{
	clientfield::register( "missile", "bo1_bouncingbetty_state", VERSION_SHIP, 2, "int" );
	clientfield::register( "scriptmover", "bo1_bouncingbetty_state", VERSION_SHIP, 2, "int" );
}
...
{
	watcher = self weaponobjects::createProximityWeaponObjectWatcher( "bo1_bouncingbetty", self.team );
...
function trackBouncingBettyAsUsed()
{
	if ( IsPlayer( self ) )
	{
		self AddWeaponStat( GetWeapon( "bo1_bouncingbetty" ), "used", 1 );
	}
}
...
function spawnMineMover() // self == betty ent
{
	...
	self clientfield::set( "bo1_bouncingbetty_state", BOUNCINGBETTY_DEPLOYING );
...
{	
	...
	self clientfield::set( "bo1_bouncingbetty_state", BOUNCINGBETTY_DETONATING );
